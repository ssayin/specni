...
public:
	...
	explicit constexpr Cyclic(T o);

	/*explicit*/ constexpr operator T() const { return this->m; }
	...
	template<std::floating_point U, int Mn, int Mx>
	constexpr friend std::tuple<Cyclic, Cyclic>
	ArcPair(Cyclic
	lhs,
			Cyclic rhs
	);
...
template<std::floating_point T, int MIN, int MAX>
constexpr Cyclic<T, MIN, MAX>::Cyclic(T
o)
{
	this->
			m = o;
...
ArcPair(Cyclic<T, MIN, MAX>
lhs,
		Cyclic<T, MIN, MAX> rhs
)
{
	constexpr T
			epsilon = std::numeric_limits<T>::epsilon()*1000;
	Cyclic<T, MIN, MAX> sub = lhs-rhs;
	return {
			sub,
			Cyclic<T, MIN, MAX>(MAX
					-static_cast
							<T>(sub)
			)};
}
...
using Declination = Cyclic<double, -90, 90>;
using Latitude = Cyclic<double, -90, 90>;
} // namespace specni
