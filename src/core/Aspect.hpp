...
struct DefaultOrbConfig {
	static constexpr auto Get(swe::Ipl body)
	-> int
	{
		switch (body) {
		case swe::Ipl::Sun:
...
struct OrbPartileConfig {
	static constexpr auto Get([[maybe_unused]] swe::Ipl body)
		...
		const swe::Planet& p2)
{
	...
	double actualArc = Min(ArcPair(p1.longitude(), p2.longitude()));
	auto optIt = detail::ClosestBoundary(maxAllowed, actualArc);

	if (!optIt.has_value()) // FIXME: second check is unnecessary
		return std::nullopt;

	double futureArc = Min(ArcPair(p1.longitude()+p1.speedLongitude(),
			p2.longitude()+p2.speedLongitude()));
...
{
	double maxAllowed = GetMaxOrb<OrbConfig>(p1, p2);
	auto sum = p1.declination() + p2.declination();
	auto sub = p1.declination() + p2.declination();
	Type type;
